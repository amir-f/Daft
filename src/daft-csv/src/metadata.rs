...
use crate::{compression::CompressionCodec, CsvParseOptions};
    ...
    R: AsyncRead + Unpin + Send,
{
    ...
    let (fields, total_bytes_read, num_records_read, mean_size, std_size) = infer_schema(
        &mut reader,
        None,
        max_bytes,
        parse_options.has_header,
        &infer,
    )
    .await?;
    Ok((
...
mod tests {
    ...
    fn test_csv_schema_local_delimiter() -> DaftResult<()> {
        ...
        let (schema, total_bytes_read, num_records_read, _, _) = read_csv_schema(
            file.as_ref(),
            Some(CsvParseOptions::new_internal(true, b'|')),
            None,
            io_client.clone(),
            None,
        )?;
